...
static bool start_transfer(uint16_t addr, uint8_t addr_len)
{
    ...
    if (addr_len == 2) {
        ...
        while (!(IFG2 & UCTXIFG)); /* Wait for byte to be sent */
        /*If the slave does not acknowledge the transmitted data, the not-acknowledge interrupt flag UCNACKIFG is set*/
        if (USCI_REG(EUSCI_SEL,STAT) & UCNACKIFG) return false;
    }
...
{
    USCI_REG(EUSCI_SEL,CTL1) |= UCTXSTP; //Setting UCTXSTP generates a STOP condition after the next acknowledge from the slave 
    while (USCI_REG(EUSCI_SEL,CTL1) & UCTXSTP); // wait for the end of the communication
}
...
bool i2c_read_core(const uint16_t addr, uint8_t addr_len, uint8_t *data, uint16_t data_len)
{
    ...
    while (USCI_REG(EUSCI_SEL,CTL1) & UCTXSTT); /* Wait for start condition to be received */
    if (USCI_REG(EUSCI_SEL,STAT) & UCNACKIFG) return false; /*If the slave does not acknowledge the transmitted data, the not-acknowledge interrupt flag UCNACKIFG is set*/
    for(uint16_t i = 0; i < data_len; ++i){
        if (i == data_len-1) {
            USCI_REG(EUSCI_SEL,CTL1) |= UCTXSTP; /* Send stop before reading the last byte */
        }
        while (!(IFG2 & USCI_REG(EUSCI_SEL,RXIFG))); /*The UCRXIFG0 interrupt flag is set when a character is received and loaded into UCBxRXBUF*/
