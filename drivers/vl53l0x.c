...
/* ─────────────────────────────────────────────────────────────────────────
 * Generic write-sequence runner for consecutive register writes, similar to the one of ST API.
 * ───────────────────────────────────────────────────────────────────────── */
static bool run_write_sequence(const uint8_t *p)
{
    while (p[0] != 0x00) {
        ...
        for (uint8_t i = 0; i < num_bytes; ++i) data[i] = p[2 + i];
...
static const uint8_t data_init_seq[] = {
    0x01, REG_I2C_MODE, 0x00,
    0x01, REG_POWER_MANAGEMENT_GO1_POWER_FORCE, 0x01,
    0x01, REG_INTERNAL_TUNING_2, 0x01,
    0x01, REG_SYSRANGE_START, 0x00,
    0x00, 0x00, 0x00
};
...
static bool data_init()
{
    ...
    if (!run_write_sequence(data_init_seq)) return false;
...
{
    static const uint8_t spad_nvm_setup_seq[] = {
        0x01, REG_POWER_MANAGEMENT_GO1_POWER_FORCE, 0x01,
        0x01, REG_INTERNAL_TUNING_2, 0x01,
        0x01, REG_SYSRANGE_START, 0x00,
        0x01, REG_INTERNAL_TUNING_2, 0x06,
        0x00, 0x00, 0x00
    };

    static const uint8_t spad_nvm_pre_strobe_seq[] = {
        0x01, REG_INTERNAL_TUNING_2, 0x07,
        0x01, REG_SYSTEM_HISTOGRAM_BIN, 0x01,
        0x01, REG_POWER_MANAGEMENT_GO1_POWER_FORCE, 0x01,
        0x01, REG_NVM_ADDR, 0x6B,
        0x00, 0x00, 0x00
    };

    static const uint8_t spad_nvm_restore_seq[] = {
        0x01, REG_INTERNAL_TUNING_2, 0x01,
        0x01, REG_SYSRANGE_START, 0x01,
        0x01, REG_INTERNAL_TUNING_2, 0x00,
        0x01, REG_POWER_MANAGEMENT_GO1_POWER_FORCE, 0x00,
        0x00, 0x00, 0x00
    };

    if (!run_write_sequence(spad_nvm_setup_seq)) return false;

    uint8_t  tmp_data8  = 0;
    if (!i2c_read(REG_NVM_READ_STROBE, 1, &tmp_data8, 1)) return false;
    if (!i2c_write(REG_NVM_READ_STROBE, 1, (uint8_t[]){tmp_data8 | 0x04}, 1)) return false;

    if (!run_write_sequence(spad_nvm_pre_strobe_seq)) return false;

    if (!read_strobe()) return false;

    uint32_t tmp_data32 = 0;
    uint8_t  buf[4] = {0};
    if (!i2c_read(REG_NVM_READ_DATA, 1, buf, 4)) return false;

    tmp_data32   = ((uint32_t)buf[0] << 24) | ((uint32_t)buf[1] << 16)
                 | ((uint32_t)buf[2] <<  8) |  buf[3];
    *spad_count  = (tmp_data32 >>  8) & 0x7F;
    *spad_type   = (tmp_data32 >> 15) & 0x01;

    if (!i2c_write(REG_SYSTEM_HISTOGRAM_BIN, 1, (uint8_t[]){0x00}, 1)) return false;
    if (!i2c_write(REG_INTERNAL_TUNING_2,    1, (uint8_t[]){0x06}, 1)) return false;

    if (!i2c_read(REG_NVM_READ_STROBE, 1, &tmp_data8, 1)) return false;

    if (!i2c_write(REG_NVM_READ_STROBE, 1, (uint8_t[]){tmp_data8 & 0xFB}, 1)) return false;

    if (!run_write_sequence(spad_nvm_restore_seq)) return false;

    if (!i2c_read(REG_GLOBAL_CONFIG_SPAD_ENABLES_REF_0, 1, good_spad_map, 6)) return false;
...
static bool set_spads_from_nvm()
{
    ...
    static const uint8_t set_spad_nvm_seq[] = {
        0x01, REG_INTERNAL_TUNING_2, 0x01,
        0x01, REG_DYNAMIC_SPAD_REF_EN_START_OFFSET, 0x00,
        0x01, REG_DYNAMIC_SPAD_NUM_REQUESTED_REF_SPAD, 0x2C,
        0x01, REG_INTERNAL_TUNING_2, 0x00,
        0x01, REG_GLOBAL_CONFIG_REF_EN_START_SELECT, SPAD_START_SELECT,
        0x00, 0x00, 0x00
    };

    if (!run_write_sequence(set_spad_nvm_seq)) return false;
...
{    
    static const uint8_t default_tuning_seq[] = {
        0x01, REG_INTERNAL_TUNING_2, 0x01,
        0x01, REG_SYSRANGE_START, 0x00,

        0x01, REG_INTERNAL_TUNING_2, 0x00,
        0x01, REG_SYSTEM_RANGE_CONFIG, 0x00,
        0x01, 0x10, 0x00,
        0x01, REG_SYSTEM_GROUPED_PARAM_HOLD, 0x00,

        0x01, 0x24, 0x01,
        0x01, 0x25, 0xFF,
        0x01, REG_OSC_CALIBRATE_VAL, 0x00,

        0x01, REG_INTERNAL_TUNING_2, 0x01,
        0x01, REG_DYNAMIC_SPAD_NUM_REQUESTED_REF_SPAD, 0x2C,
        0x01, REG_FINAL_RANGE_CONFIG_VALID_PHASE_HIGH, 0x00,
        0x01, REG_ALGO_PHASECAL_CONFIG_TIMEOUT, 0x20,

        0x01, REG_INTERNAL_TUNING_2, 0x00,
        0x01, REG_ALGO_PHASECAL_CONFIG_TIMEOUT, 0x09,
        0x01, REG_HISTOGRAM_CONFIG_READOUT_CTRL, 0x00,
        0x01, REG_ALGO_PHASECAL_CONFIG, 0x04,
        0x01, REG_GLOBAL_CONFIG_VCSEL_WIDTH, 0x03,
        0x01, REG_UNKNOWN_0x40, 0x83,
        0x01, REG_MSRC_CONFIG_TIMEOUT_MACROP, 0x25,
        0x01, REG_MSRC_CONFIG_CONTROL, 0x00,
        0x01, REG_PRE_RANGE_CONFIG_MIN_SNR, 0x00,
        0x01, REG_PRE_RANGE_CONFIG_VCSEL_PERIOD, 0x06,
        0x01, REG_PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI, 0x00,
        0x01, REG_PRE_RANGE_CONFIG_TIMEOUT_MACROP_LO, 0x96,
        0x01, REG_PRE_RANGE_CONFIG_VALID_PHASE_LOW, 0x08,
        0x01, REG_PRE_RANGE_CONFIG_VALID_PHASE_HIGH, 0x30,
        0x01, REG_PRE_RANGE_CONFIG_SIGMA_THRESH_HI, 0x00,
        0x01, REG_PRE_RANGE_CONFIG_SIGMA_THRESH_LO, 0x00,
        0x01, REG_PRE_RANGE_MIN_COUNT_RATE_RTN_LIMIT, 0x00,
        0x01, REG_UNKNOWN_0x65, 0x00,
        0x01, REG_UNKNOWN_0x66, 0xA0,

        0x01, REG_INTERNAL_TUNING_2, 0x01,
        0x01, REG_VHV_CONFIG_TIMEOUT_MACROP, 0x32,
        0x01, REG_FINAL_RANGE_CONFIG_VALID_PHASE_LOW, 0x14,
        0x01, REG_UNKNOWN_0x49, 0xFF,
        0x01, REG_UNKNOWN_0x4A, 0x00,

        0x01, REG_INTERNAL_TUNING_2, 0x00,
        0x01, REG_RESULT_CORE_AMBIENT_WINDOW_EVENTS_RTN, 0x0A,
        0x01, REG_RESULT_CORE_RANGING_TOTAL_EVENTS_RTN, 0x00,
        0x01, REG_RESULT_PEAK_SIGNAL_RATE_REF, 0x21,

        0x01, REG_INTERNAL_TUNING_2, 0x01,
        0x01, REG_VHV_CONFIG_LOOPBOUND, 0x34,
        0x01, REG_UNKNOWN_0x42, 0x00,
        0x01, REG_FINAL_RANGE_CONFIG_MIN_COUNT_RATE_RTN_LIMIT, 0xFF,
        0x01, REG_UNKNOWN_0x45, 0x26,
        0x01, REG_MSRC_CONFIG_TIMEOUT_MACROP, 0x05,
        0x01, REG_UNKNOWN_0x40, 0x40,
        0x01, 0x0E, 0x06,
        0x01, REG_CROSSTALK_COMPENSATION_PEAK_RATE_MCPS, 0x1A,
        0x01, REG_UNKNOWN_0x43, 0x40,

        0x01, REG_INTERNAL_TUNING_2, 0x00,
        0x01, REG_ALGO_PHASECAL_CONFIG_START, 0x03,
        0x01, REG_ALGO_PHASECAL_CONFIG_END, 0x44,

        0x01, REG_INTERNAL_TUNING_2, 0x01,
        0x01, REG_ALGO_PHASECAL_CONFIG, 0x04,
        0x01, REG_UNKNOWN_0x4B, 0x09,
        0x01, REG_UNKNOWN_0x4C, 0x05,
        0x01, REG_UNKNOWN_0x4D, 0x04,

        0x01, REG_INTERNAL_TUNING_2, 0x00,
        0x01, REG_FINAL_RANGE_CONFIG_MIN_COUNT_RATE_RTN_LIMIT, 0x00,
        0x01, REG_UNKNOWN_0x45, 0x20,
        0x01, REG_FINAL_RANGE_CONFIG_VALID_PHASE_LOW, 0x08,
        0x01, REG_FINAL_RANGE_CONFIG_VALID_PHASE_HIGH, 0x28,
        0x01, REG_FINAL_RANGE_CONFIG_MIN_SNR, 0x00,
        0x01, REG_FINAL_RANGE_CONFIG_VCSEL_PERIOD, 0x04,
        0x01, REG_FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI, 0x01,
        0x01, REG_FINAL_RANGE_CONFIG_TIMEOUT_MACROP_LO, 0xFE,
        0x01, REG_RESULT_CORE_AMBIENT_WINDOW_EVENTS_REF, 0x00,
        0x01, REG_RESULT_CORE_RANGING_TOTAL_EVENTS_REF, 0x00,
        ...
        0x01, REG_INTERNAL_TUNING_2, 0x01,
        0x01, REG_INTERNAL_TUNING_PAGE_REG, 0x01,
        0x01, REG_SYSRANGE_START, 0x01,
    ...
    return run_write_sequence(default_tuning_seq);
}
...
{    
    static const uint8_t interrupt_threshold_tuning[] = {
        ...
        0x01, REG_INTERNAL_TUNING_2, 0x01,
        0x01, 0x4f, 0x02,
        0x01, REG_INTERNAL_TUNING_2, 0x0E,
        0x01, REG_SYSRANGE_START, 0x03,
        0x01, 0x01, 0x84,
        0x01, 0x02, 0x0A,
        0x01, 0x03, 0x03,
        0x01, 0x04, 0x08,
        ...
        0x01, 0x0C, 0x00,
        0x01, 0x0D, 0xD5,
        0x01, REG_SYSTEM_THRESH_LOW, 0x18,
        0x01, 0x0F, 0x12,
        0x01, 0x10, 0x01,
        0x01, 0x11, 0x82,
        0x01, 0x12, 0x00,
        ...
        0x01, 0x7A, 0x00,
        0x01, 0x7B, 0x04,
        0x01, REG_INTERNAL_TUNING_2, 0x04,
        0x01, 0x79, 0x1D,
        0x01, 0x7B, 0x27,
        0x01, 0x96, 0x0E,
        0x01, 0x97, 0xFE,
    ...
    return run_write_sequence(interrupt_threshold_tuning);
}
...
bool xshut_toggle(bool state)
{
    if(state) 
    {
        ...
        return WAIT_UNTIL(device_is_booted(), 100000);
    }
    else 
    {
...
bool vl53l0x_start_continuous(void)
{
    ...
    static const uint8_t start_conitnuous_seq_pre[] = {
        ...
        0x01, REG_INTERNAL_TUNING_2, 0x01,
        0x01, REG_SYSRANGE_START, 0x00,
        0x01, REG_INTERNAL_TUNING_1, 0x00,
        0x00, 0x00, 0x00
    };

    if(!run_write_sequence(start_conitnuous_seq_pre)) goto CLEANUP;
    ...
    static const uint8_t start_conitnuous_seq_post[] = {
        0x01, REG_SYSRANGE_START, 0x01,
        0x01, REG_INTERNAL_TUNING_2, 0x00,
        0x01, REG_POWER_MANAGEMENT_GO1_POWER_FORCE, 0x00,
        0x00, 0x00, 0x00
    };
    if(!run_write_sequence(start_conitnuous_seq_post)) goto CLEANUP;
